...
Includes support for the netbox-software-tracker plugin for firmware version tracking.
"""
...
class NetBoxClient:
    ...
    Includes integration with netbox-software-tracker plugin.
    """
    ...
    def get_or_create_node(self, fabric_id: int, node_id: int, **kwargs) -> Tuple[Any, bool]:
        ...
        try:
            ...
            create_data = {'aci_fabric': fabric_id, 'node_id': node_id, 'name': name, **create_kwargs}
    ...
    def get_dcim_device_by_name(self, name: str) -> Optional[Any]:
        """Get a DCIM device by its name."""
        try:
    ...
    def get_device_type_by_model(self, model: str) -> Optional[Any]:
        """Get a device type by model name."""
        try:
            return self.api.dcim.device_types.get(model=model)
        except Exception:
            return None
    ...
    # =========================================================================
    # Software Tracker Plugin Operations
    # =========================================================================
    #
    # Plugin: netbox-software-tracker (https://pypi.org/project/netbox-software-tracker/)
    # API base: /api/plugins/netbox_software_tracker/
    # Endpoints:
    #   - software-image/  (fields: version, filename, md5sum, comments)
    #   - golden-image/    (links software-image to device types)
    #
    # Uses direct HTTP requests since pynetbox may not auto-discover
    # plugin endpoints reliably (the plugin registers under "null" key).
    # =========================================================================

    def _software_tracker_request(self, method: str, endpoint: str,
                                   params: Dict = None,
                                   json_data: Dict = None,
                                   item_id: int = None) -> Optional[Any]:
        """
        Make a request to the software tracker plugin API.

        Args:
            method: HTTP method (get, post, patch, delete)
            endpoint: Endpoint name (e.g. 'software-image', 'golden-image')
            params: Query parameters for GET requests
            json_data: JSON body for POST/PATCH requests
            item_id: Optional object ID for detail endpoints
        ...
        base = self.url.rstrip('/')
        url = f"{base}/api/plugins/netbox_software_tracker/{endpoint}/"
        if item_id:
            url = f"{base}/api/plugins/netbox_software_tracker/{endpoint}/{item_id}/"
        ...
        try:
            ...
            logger.error(f"Software tracker API request failed: {e}")
            return None

    def get_or_create_software_image(self, version: str,
                                      **kwargs) -> Tuple[Any, bool]:
        """
        Get or create a software-image entry in the software tracker plugin.

        Args:
            version: Software version string (e.g. "6.1(4h)"), max 32 chars
            **kwargs: Optional fields: filename, md5sum, comments

        Returns:
            Tuple of (software_image_object, created_bool)
        """
        from types import SimpleNamespace

        # Search for existing by version
        response = self._software_tracker_request(
            'get', 'software-image', params={'version': version}
        )
            ...
            try:
                ...
                for item in results:
                    if item.get('version') == version:
                        obj = self._wrap_software_tracker_obj(item, 'software-image')
                        return obj, False
            except Exception as e:
                logger.debug(f"Error parsing software-image search response: {e}")

        # Not found - create new
        create_data = {'version': version, **kwargs}
        response = self._software_tracker_request(
            'post', 'software-image', json_data=create_data
        )

        if response and response.status_code in (200, 201):
            obj_data = response.json()
            obj = self._wrap_software_tracker_obj(obj_data, 'software-image')
            return obj, True

        status = response.status_code if response else 'no response'
        body = response.text[:200] if response else ''
        raise Exception(
            f"Failed to get/create software-image version '{version}': "
            f"{status} - {body}"
        )

    def update_software_image(self, sw_image: Any, updates: Dict,
                               verify: bool = True) -> Tuple[bool, bool]:
        """Update a software-image entry with given attributes."""
        return self._update_if_changed(sw_image, updates, verify)

    def assign_golden_image(self, software_image_id: int,
                             device_type_id: int) -> bool:
        """
        Assign a software-image as the golden image for a device type.

        Args:
            software_image_id: ID of the software-image entry
            device_type_id: ID of the DCIM device type

        Returns:
            True if created, False if already exists or failed
        """
        # Check for existing assignments for this device type
        response = self._software_tracker_request(
            'get', 'golden-image',
        ...
        if response and response.status_code == 200:
            try:
                ...
                results = data.get('results', data if isinstance(data, list) else [])
                for item in results:
                    # Check if this software is already assigned
                    sw = item.get('software', {})
                    sw_id = sw.get('id') if isinstance(sw, dict) else sw
                    if sw_id == software_image_id:
                        return False  # Already assigned
            except Exception:
                pass

        # Create new assignment
        # golden-image requires: device_type (ID) and software (ID)
        response = self._software_tracker_request(
            'post', 'golden-image',
            json_data={
        ...
        if response:
            logger.debug(
                f"Golden image assignment failed: "
                f"{response.status_code} - {response.text[:200]}"
            )
        return False

    def _wrap_software_tracker_obj(self, data: Dict,
                                    endpoint: str) -> Any:
        """
        Wrap a software tracker API response dict in a SimpleNamespace
        with an update() method for compatibility with _update_if_changed.
        """
        from types import SimpleNamespace
        ...
        def _update(updates):
            response = self._software_tracker_request(
                'patch', endpoint, json_data=updates, item_id=obj_id
            )
            if response and response.status_code in (200, 201):
